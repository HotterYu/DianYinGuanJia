...
public class SearchView extends LinearLayout {
     ...
     * 作用：初始化搜索框
     */
    public void init(String tblName){

        // 1. 初始化UI组件->>关注c
        initView();

        // 2. 实例化数据库SQLiteOpenHelper子类对象
        helper = new RecordSQLiteOpenHelper(context,tblName);
    ...
    {
        return svSearchRecord.isShown() || tv_clear.isShown();
    }

    public void showRecordView(boolean show)
    {
        if(show)
        {
            tv_clear.setVisibility(VISIBLE);
            svSearchRecord.setVisibility(VISIBLE);
        }
        else
        {
